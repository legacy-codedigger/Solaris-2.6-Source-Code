
/****************************************************************
 *
 * Name      : pfgUseDisks.c
 *
 * File is generated by TeleUSE
 * Version : TeleUSE v3.0.2 / Solaris 2.4
 *
 * Copyright (c) 1994-1995 by Sun MicroSystems, Inc.
 * All rights reserved.
 *
 ****************************************************************/

#include <X11/Intrinsic.h>
#include <X11/StringDefs.h>
#include <X11/Shell.h>
#include <Xm/Xm.h>
#include <Xm/Protocols.h>

#include "tu_runtime.h"

#include <stdarg.h>

#include <stdio.h>
#include <stdlib.h>
#include "pfgUseDisks_ui.h"


#include <Xm/Form.h>
#include <Xm/Label.h>
#include <Xm/List.h>
#include <Xm/MessageB.h>
#include <Xm/MwmUtil.h>
#include <Xm/PushB.h>
#include <Xm/RowColumn.h>
#include <Xm/ScrolledW.h>
#include <Xm/Text.h>


/* Local variables used when selecting screen & display */
static Display * sDisplay = NULL;
static Screen  * sScreen  = NULL;

/* forward declaration */
static void class_init(void);

extern void disksGobackCB(Widget widget,
                          tu_ccb_arg_p closure,
                          XtPointer calldata);
extern void pfgExit(Widget widget,
                    tu_ccb_arg_p closure,
                    XtPointer calldata);
extern void changeBootCB(Widget widget,
                         tu_ccb_arg_p closure,
                         XtPointer calldata);
extern void unregister_as_dropsite(Widget widget,
                                   tu_ccb_arg_p closure,
                                   XtPointer calldata);
extern void pfgHelp(Widget widget,
                    XtPointer closure,
                    XtPointer calldata);


/****************************************************************
 * get_constraint_widget:
 ****************************************************************/
static Widget get_constraint_widget(Widget child, Widget parent)
{
  Widget w;

  w = child;
  while (XtParent(w) != parent)
    w = XtParent(w);
  return (w);
}


/****************************************************************
 * put_client_data_arg:
 *    set up a client data argument.
 ****************************************************************/
static tu_ccb_arg_p put_client_data_arg(unsigned int cnt, ...)
{
  va_list pvar;
  int i;
  tu_ccb_arg_p rv;

  rv = (tu_ccb_arg_p) malloc(cnt*sizeof(tu_ccb_arg_t));
  if (rv == NULL) return NULL;

  va_start(pvar, cnt);

  for (i=0;i<cnt;i++) {
    rv[i].name = va_arg(pvar, char *);
    rv[i].value = va_arg(pvar, char *);
    rv[i].next = &rv[i+1];
  }
  va_end(pvar);
  rv[cnt-1].next = NULL;
  return rv;
}



/****************************************************************
 * class_init:
 *     Initializes the classes used by the user interface.
 ****************************************************************/
static void class_init(void)
{
  static int inited = 0;

  if (inited != 0)
    return;

  inited = 1;
  sDisplay = sDisplay;

  XtInitializeWidgetClass(xmFormWidgetClass);
  XtInitializeWidgetClass(xmTextWidgetClass);
  XtInitializeWidgetClass(xmLabelWidgetClass);
  XtInitializeWidgetClass(xmScrolledWindowWidgetClass);
  XtInitializeWidgetClass(xmRowColumnWidgetClass);
  XtInitializeWidgetClass(xmPushButtonWidgetClass);
  XtInitializeWidgetClass(xmMessageBoxWidgetClass);
}



/****************************************************************
 *
 *  Main C code for presentation component
 *
 ****************************************************************/

/**************************************************************
 * tu_usedisks_dialog_widget:
 **************************************************************/
Widget tu_usedisks_dialog_widget(char    * name,
                                 Widget    parent,
                                 Widget ** warr_ret)
{
  Arg args[26];
  Arg pargs[26];
  Widget tmpw;
  Widget tmpw1;
  Widget widget_array[33];
  XtTranslations ttbl;
  int n;
  int pn;
  /* Make sure the classes used are initialized */
  class_init();

  /***************** object of type : XmFormDialog *****************/
  widget_array[WI_USEDISKS_DIALOG] =
    XmCreateFormDialog(parent, name, NULL, 0);

  /***************** panelhelpText : XmText *****************/
  n = 0;
  XtSetArg(args[n], XmNautoShowCursorPosition, False); n++;
  XtSetArg(args[n], XmNcursorPositionVisible, False); n++;
  XtSetArg(args[n], XmNeditMode, XmMULTI_LINE_EDIT); n++;
  XtSetArg(args[n], XmNtraversalOn, False); n++;
  XtSetArg(args[n], XmNresizeHeight, True); n++;
  XtSetArg(args[n], XmNwordWrap, True); n++;
  XtSetArg(args[n], XmNshadowThickness, 0); n++;
  XtSetArg(args[n], XmNmarginWidth, 18); n++;
  XtSetArg(args[n], XmNmarginHeight, 18); n++;
  XtSetArg(args[n], XmNvalue, "message_text"); n++;
  widget_array[WI_PANELHELPTEXT] =
    XmCreateText(widget_array[WI_USEDISKS_DIALOG], "panelhelpText", args, n);

  /***************** form : XmForm *****************/
  n = 0;
  XtSetArg(args[n], XmNfractionBase, 9); n++;
  XtSetArg(args[n], XmNmarginWidth, 20); n++;
  XtSetArg(args[n], XmNrubberPositioning, False); n++;
  XtSetArg(args[n], XmNresizePolicy, XmRESIZE_ANY); n++;
  widget_array[WI_FORM] =
    XmCreateForm(widget_array[WI_USEDISKS_DIALOG], "form", args, n);

  /***************** availableLabel : XmLabel *****************/
  widget_array[WI_AVAILABLELABEL] =
    XmCreateLabel(widget_array[WI_FORM], "availableLabel", NULL, 0);

  /***************** availableScrolledList : XmScrolledList *****************/
  n = 0;
  pn = 0;
  XtSetArg(args[n], XmNlistSizePolicy, XmCONSTANT); n++;
  XtSetArg(pargs[pn], XmNresizable, False); pn++;
  widget_array[WI_AVAILABLESCROLLEDLIST] =
    XmCreateScrolledList(widget_array[WI_FORM], "availableScrolledList", args, n);
  tmpw = get_constraint_widget(widget_array[WI_AVAILABLESCROLLEDLIST], widget_array[WI_FORM]);
  if (tmpw)
    XtSetValues(tmpw, pargs, pn);

  /***************** totalAvailableLabel : XmLabel *****************/
  n = 0;
  XtSetArg(args[n], XmNalignment, XmALIGNMENT_END); n++;
  XtSetArg(args[n], XmNresizable, True); n++;
  widget_array[WI_TOTALAVAILABLELABEL] =
    XmCreateLabel(widget_array[WI_FORM], "totalAvailableLabel", args, n);

  /***************** totalAvailableValue : XmLabel *****************/
  n = 0;
  XtSetArg(args[n], XmNalignment, XmALIGNMENT_END); n++;
  XtSetArg(args[n], XmNresizable, True); n++;
  widget_array[WI_TOTALAVAILABLEVALUE] =
    XmCreateLabel(widget_array[WI_FORM], "totalAvailableValue", args, n);

  /***************** buttonForm : XmRowColumn *****************/
  n = 0;
  XtSetArg(args[n], XmNentryAlignment, XmALIGNMENT_CENTER); n++;
  XtSetArg(args[n], XmNmarginWidth, 10); n++;
  XtSetArg(args[n], XmNspacing, 10); n++;
  widget_array[WI_BUTTONFORM] =
    XmCreateRowColumn(widget_array[WI_FORM], "buttonForm", args, n);

  /***************** addButton : XmPushButton *****************/
  n = 0;
  XtSetArg(args[n], XmNwidth, 50); n++;
  widget_array[WI_ADDBUTTON] =
    XmCreatePushButton(widget_array[WI_BUTTONFORM], "addButton", args, n);

  /***************** removeButton : XmPushButton *****************/
  n = 0;
  XtSetArg(args[n], XmNwidth, 50); n++;
  widget_array[WI_REMOVEBUTTON] =
    XmCreatePushButton(widget_array[WI_BUTTONFORM], "removeButton", args, n);

  /***************** autoAddButton : XmPushButton *****************/
  n = 0;
  XtSetArg(args[n], XmNwidth, 50); n++;
  widget_array[WI_AUTOADDBUTTON] =
    XmCreatePushButton(widget_array[WI_BUTTONFORM], "autoAddButton", args, n);

  /***************** removeAllButton : XmPushButton *****************/
  n = 0;
  XtSetArg(args[n], XmNwidth, 50); n++;
  widget_array[WI_REMOVEALLBUTTON] =
    XmCreatePushButton(widget_array[WI_BUTTONFORM], "removeAllButton", args, n);

  /***************** solarButton : XmPushButton *****************/
  n = 0;
  XtSetArg(args[n], XmNwidth, 50); n++;
  widget_array[WI_SOLARBUTTON] =
    XmCreatePushButton(widget_array[WI_BUTTONFORM], "solarButton", args, n);

  /***************** selectedLabel : XmLabel *****************/
  widget_array[WI_SELECTEDLABEL] =
    XmCreateLabel(widget_array[WI_FORM], "selectedLabel", NULL, 0);

  /***************** selectedScrolledList : XmScrolledList *****************/
  n = 0;
  pn = 0;
  XtSetArg(args[n], XmNlistSizePolicy, XmCONSTANT); n++;
  XtSetArg(pargs[pn], XmNresizable, False); pn++;
  widget_array[WI_SELECTEDSCROLLEDLIST] =
    XmCreateScrolledList(widget_array[WI_FORM], "selectedScrolledList", args, n);
  tmpw = get_constraint_widget(widget_array[WI_SELECTEDSCROLLEDLIST], widget_array[WI_FORM]);
  if (tmpw)
    XtSetValues(tmpw, pargs, pn);

  /***************** rowColumn : XmRowColumn *****************/
  n = 0;
  XtSetArg(args[n], XmNentryAlignment, XmALIGNMENT_END); n++;
  widget_array[WI_ROWCOLUMN] =
    XmCreateRowColumn(widget_array[WI_FORM], "rowColumn", args, n);

  /***************** recommendLabel : XmLabel *****************/
  widget_array[WI_RECOMMENDLABEL] =
    XmCreateLabel(widget_array[WI_ROWCOLUMN], "recommendLabel", NULL, 0);

  /***************** minimumLabel : XmLabel *****************/
  widget_array[WI_MINIMUMLABEL] =
    XmCreateLabel(widget_array[WI_ROWCOLUMN], "minimumLabel", NULL, 0);

  /***************** useTotalLabel : XmLabel *****************/
  widget_array[WI_USETOTALLABEL] =
    XmCreateLabel(widget_array[WI_ROWCOLUMN], "useTotalLabel", NULL, 0);

  /***************** rowColumn1 : XmRowColumn *****************/
  n = 0;
  XtSetArg(args[n], XmNentryAlignment, XmALIGNMENT_END); n++;
  widget_array[WI_ROWCOLUMN1] =
    XmCreateRowColumn(widget_array[WI_FORM], "rowColumn1", args, n);

  /***************** recommendValue : XmLabel *****************/
  widget_array[WI_RECOMMENDVALUE] =
    XmCreateLabel(widget_array[WI_ROWCOLUMN1], "recommendValue", NULL, 0);

  /***************** minimumValue : XmLabel *****************/
  widget_array[WI_MINIMUMVALUE] =
    XmCreateLabel(widget_array[WI_ROWCOLUMN1], "minimumValue", NULL, 0);

  /***************** useTotalValue : XmLabel *****************/
  widget_array[WI_USETOTALVALUE] =
    XmCreateLabel(widget_array[WI_ROWCOLUMN1], "useTotalValue", NULL, 0);

  /***************** boot_device_form : XmForm *****************/
  n = 0;
  XtSetArg(args[n], XmNrubberPositioning, False); n++;
  XtSetArg(args[n], XmNresizePolicy, XmRESIZE_ANY); n++;
  XtSetArg(args[n], XmNresizable, True); n++;
  widget_array[WI_BOOT_DEVICE_FORM] =
    XmCreateForm(widget_array[WI_FORM], "boot_device_form", args, n);

  /***************** boot_label : XmLabel *****************/
  n = 0;
  XtSetArg(args[n], XmNalignment, XmALIGNMENT_BEGINNING); n++;
  XtSetArg(args[n], XmNresizable, True); n++;
  widget_array[WI_BOOT_LABEL] =
    XmCreateLabel(widget_array[WI_BOOT_DEVICE_FORM], "boot_label", args, n);

  /***************** changeButton : XmPushButton *****************/
  n = 0;
  XtSetArg(args[n], XmNresizable, True); n++;
  widget_array[WI_CHANGEBUTTON] =
    XmCreatePushButton(widget_array[WI_BOOT_DEVICE_FORM], "changeButton", args, n);

  /***************** messageBox : XmMessageBox *****************/
  n = 0;
  XtSetArg(args[n], XmNdialogType, XmDIALOG_TEMPLATE); n++;
  widget_array[WI_MESSAGEBOX] =
    XmCreateMessageBox(widget_array[WI_USEDISKS_DIALOG], "messageBox", args, n);

  /***************** continueButton : XmPushButton *****************/
  widget_array[WI_CONTINUEBUTTON] =
    XmCreatePushButton(widget_array[WI_MESSAGEBOX], "continueButton", NULL, 0);

  /***************** gobackButton : XmPushButton *****************/
  widget_array[WI_GOBACKBUTTON] =
    XmCreatePushButton(widget_array[WI_MESSAGEBOX], "gobackButton", NULL, 0);

  /***************** exitButton : XmPushButton *****************/
  widget_array[WI_EXITBUTTON] =
    XmCreatePushButton(widget_array[WI_MESSAGEBOX], "exitButton", NULL, 0);

  /***************** helpButton : XmPushButton *****************/
  widget_array[WI_HELPBUTTON] =
    XmCreatePushButton(widget_array[WI_MESSAGEBOX], "helpButton", NULL, 0);

  /***************** button5 : XmPushButton *****************/
  widget_array[WI_BUTTON5] =
    XmCreatePushButton(widget_array[WI_MESSAGEBOX], "button5", NULL, 0);

  /* Terminate the widget array */
  widget_array[32] = NULL;


  /***************** panelhelpText : XmText *****************/
  n = 0;
  ttbl = XtParseTranslationTable("#override\n\
~Ctrl ~Meta<BtnDown>:\n\
~Ctrl ~Meta<BtnUp>:");
  XtOverrideTranslations(widget_array[WI_PANELHELPTEXT], ttbl);
  unregister_as_dropsite(widget_array[WI_PANELHELPTEXT],
                         NULL,
                         NULL);

  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM); n++;
  XtSetValues(widget_array[WI_PANELHELPTEXT], args, n);

  XtManageChild(widget_array[WI_PANELHELPTEXT]);

  /***************** form : XmForm *****************/
  n = 0;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNtopWidget, widget_array[WI_PANELHELPTEXT]); n++;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNbottomWidget, widget_array[WI_MESSAGEBOX]); n++;
  XtSetValues(widget_array[WI_FORM], args, n);


  /***************** availableLabel : XmLabel *****************/
  n = 0;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION); n++;
  XtSetArg(args[n], XmNrightPosition, 4); n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
  XtSetValues(widget_array[WI_AVAILABLELABEL], args, n);

  XtManageChild(widget_array[WI_AVAILABLELABEL]);

  /***************** availableScrolledList : XmScrolledList *****************/
  pn = 0;
  XtSetArg(pargs[pn], XmNleftAttachment, XmATTACH_FORM); pn++;
  XtSetArg(pargs[pn], XmNtopAttachment, XmATTACH_WIDGET); pn++;
  XtSetArg(pargs[pn], XmNtopWidget, widget_array[WI_SELECTEDLABEL]); pn++;
  XtSetArg(pargs[pn], XmNbottomAttachment, XmATTACH_WIDGET); pn++;
  XtSetArg(pargs[pn], XmNrightAttachment, XmATTACH_POSITION); pn++;
  XtSetArg(pargs[pn], XmNrightPosition, 4); pn++;
  XtSetArg(pargs[pn], XmNbottomWidget, widget_array[WI_ROWCOLUMN1]); pn++;
  tmpw = get_constraint_widget(widget_array[WI_AVAILABLESCROLLEDLIST], widget_array[WI_FORM]);
  if (tmpw)
    XtSetValues(tmpw, pargs, pn);

  XtManageChild(widget_array[WI_AVAILABLESCROLLEDLIST]);

  /***************** totalAvailableLabel : XmLabel *****************/
  n = 0;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_NONE); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_NONE); n++;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET); n++;
  XtSetArg(args[n], XmNbottomWidget, widget_array[WI_TOTALAVAILABLEVALUE]); n++;
  XtSetArg(args[n], XmNrightWidget, widget_array[WI_TOTALAVAILABLEVALUE]); n++;
  XtSetValues(widget_array[WI_TOTALAVAILABLELABEL], args, n);

  XtManageChild(widget_array[WI_TOTALAVAILABLELABEL]);

  /***************** totalAvailableValue : XmLabel *****************/
  n = 0;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_NONE); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION); n++;
  XtSetArg(args[n], XmNrightPosition, 4); n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNtopWidget, widget_array[WI_BUTTONFORM]); n++;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE); n++;
  XtSetValues(widget_array[WI_TOTALAVAILABLEVALUE], args, n);

  XtManageChild(widget_array[WI_TOTALAVAILABLEVALUE]);

  /***************** buttonForm : XmRowColumn *****************/
  n = 0;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNtopWidget, widget_array[WI_AVAILABLELABEL]); n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION); n++;
  XtSetArg(args[n], XmNleftPosition, 4); n++;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNbottomWidget, widget_array[WI_ROWCOLUMN1]); n++;
  XtSetValues(widget_array[WI_BUTTONFORM], args, n);

  XtSetSensitive(widget_array[WI_ADDBUTTON], False);
  XtManageChild(widget_array[WI_ADDBUTTON]);
  XtSetSensitive(widget_array[WI_REMOVEBUTTON], False);
  XtManageChild(widget_array[WI_REMOVEBUTTON]);
  XtSetSensitive(widget_array[WI_AUTOADDBUTTON], False);
  XtManageChild(widget_array[WI_AUTOADDBUTTON]);
  XtSetSensitive(widget_array[WI_REMOVEALLBUTTON], False);
  XtManageChild(widget_array[WI_REMOVEALLBUTTON]);
  XtSetSensitive(widget_array[WI_SOLARBUTTON], False);
  XtManageChild(widget_array[WI_SOLARBUTTON]);
  XtManageChild(widget_array[WI_BUTTONFORM]);

  /***************** selectedLabel : XmLabel *****************/
  n = 0;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNleftPosition, 6); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNleftWidget, widget_array[WI_BUTTONFORM]); n++;
  XtSetValues(widget_array[WI_SELECTEDLABEL], args, n);

  XtManageChild(widget_array[WI_SELECTEDLABEL]);

  /***************** selectedScrolledList : XmScrolledList *****************/
  pn = 0;
  XtSetArg(pargs[pn], XmNleftAttachment, XmATTACH_WIDGET); pn++;
  XtSetArg(pargs[pn], XmNleftWidget, widget_array[WI_BUTTONFORM]); pn++;
  XtSetArg(pargs[pn], XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET); pn++;
  tmpw1 = get_constraint_widget(
widget_array[WI_AVAILABLESCROLLEDLIST], widget_array[WI_FORM]);
  XtSetArg(pargs[pn], XmNtopWidget, tmpw1); pn++;
  XtSetArg(pargs[pn], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET); pn++;
  tmpw1 = get_constraint_widget(
widget_array[WI_AVAILABLESCROLLEDLIST], widget_array[WI_FORM]);
  XtSetArg(pargs[pn], XmNbottomWidget, tmpw1); pn++;
  XtSetArg(pargs[pn], XmNrightAttachment, XmATTACH_FORM); pn++;
  tmpw = get_constraint_widget(widget_array[WI_SELECTEDSCROLLEDLIST], widget_array[WI_FORM]);
  if (tmpw)
    XtSetValues(tmpw, pargs, pn);

  XtManageChild(widget_array[WI_SELECTEDSCROLLEDLIST]);

  /***************** rowColumn : XmRowColumn *****************/
  n = 0;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNrightWidget, widget_array[WI_ROWCOLUMN1]); n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET); n++;
  XtSetArg(args[n], XmNtopWidget, widget_array[WI_ROWCOLUMN1]); n++;
  XtSetArg(args[n], XmNbottomWidget, widget_array[WI_ROWCOLUMN1]); n++;
  XtSetValues(widget_array[WI_ROWCOLUMN], args, n);

  XtManageChild(widget_array[WI_RECOMMENDLABEL]);
  XtManageChild(widget_array[WI_MINIMUMLABEL]);
  XtManageChild(widget_array[WI_USETOTALLABEL]);
  XtManageChild(widget_array[WI_ROWCOLUMN]);

  /***************** rowColumn1 : XmRowColumn *****************/
  n = 0;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNrightOffset, 20); n++;
  XtSetValues(widget_array[WI_ROWCOLUMN1], args, n);

  XtManageChild(widget_array[WI_RECOMMENDVALUE]);
  XtManageChild(widget_array[WI_MINIMUMVALUE]);
  XtManageChild(widget_array[WI_USETOTALVALUE]);
  XtManageChild(widget_array[WI_ROWCOLUMN1]);

  /***************** boot_device_form : XmForm *****************/
  n = 0;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_NONE); n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE); n++;
  XtSetArg(args[n], XmNtopWidget, widget_array[WI_TOTALAVAILABLEVALUE]); n++;
  XtSetValues(widget_array[WI_BOOT_DEVICE_FORM], args, n);


  /***************** boot_label : XmLabel *****************/
  n = 0;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE); n++;
  XtSetValues(widget_array[WI_BOOT_LABEL], args, n);

  XtManageChild(widget_array[WI_BOOT_LABEL]);

  /***************** changeButton : XmPushButton *****************/
  n = 0;
  XtAddCallback(widget_array[WI_CHANGEBUTTON],
                XmNactivateCallback,
                (XtCallbackProc)changeBootCB,
                (XtPointer)NULL);

  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET); n++;
  XtSetArg(args[n], XmNtopWidget, widget_array[WI_BOOT_LABEL]); n++;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE); n++;
  XtSetValues(widget_array[WI_CHANGEBUTTON], args, n);

  XtManageChild(widget_array[WI_CHANGEBUTTON]);
  XtManageChild(widget_array[WI_BOOT_DEVICE_FORM]);
  XtManageChild(widget_array[WI_FORM]);

  /***************** messageBox : XmMessageBox *****************/
  n = 0;
  XtSetArg(args[n], XmNdefaultButton, widget_array[WI_CONTINUEBUTTON]); n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM); n++;
  XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM); n++;
  XtSetValues(widget_array[WI_MESSAGEBOX], args, n);

  XtManageChild(widget_array[WI_CONTINUEBUTTON]);
  XtAddCallback(widget_array[WI_GOBACKBUTTON],
                XmNactivateCallback,
                (XtCallbackProc)disksGobackCB,
                (XtPointer)NULL);

  XtManageChild(widget_array[WI_GOBACKBUTTON]);
  XtAddCallback(widget_array[WI_EXITBUTTON],
                XmNactivateCallback,
                (XtCallbackProc)pfgExit,
                (XtPointer)NULL);

  XtManageChild(widget_array[WI_EXITBUTTON]);
  XtAddCallback(widget_array[WI_HELPBUTTON],
                XmNactivateCallback,
                pfgHelp,
                (XtPointer)"spotseldisk.r");

  XtManageChild(widget_array[WI_HELPBUTTON]);
  XtManageChild(widget_array[WI_MESSAGEBOX]);

  /*
   *  Allocate memory for the widget array to return
   */
  if (warr_ret != NULL) {
    *warr_ret = (Widget *) malloc(sizeof(Widget)*33);
    (void) memcpy((char *)*warr_ret,
                  (char *)widget_array,
           sizeof(Widget)*33);
  }

  /*
   *   Fix for SMI's X/NeWS server
   */
  tu_ol_fix_hierarchy(widget_array[WI_USEDISKS_DIALOG]);

  /*
   *   Invoke the create callbacks
   */
  tu_widcre_invoke_hooks(widget_array[WI_USEDISKS_DIALOG]);

  /*
   *   Return the first created widget.
   */
  return widget_array[WI_USEDISKS_DIALOG];
}



/****************************************************************
 * create_method:
 *     This function creates a widget hierarchy using the
 *     functions generated above.
 ****************************************************************/
static Widget create_method(char               * temp,
                            char               * name,
                            Widget               parent,
                            Display            * disp,
                            Screen             * screen,
                            tu_template_descr  * retval)
{
  Widget w;

  sDisplay = disp;
  sScreen = screen;

  /* check each node against its name and call its
   * create function if appropriate */
  w = NULL;
  if (strcmp(temp, "usedisks_dialog") == 0){
    w = tu_usedisks_dialog_widget(name, parent, (Widget **)retval);
  }

  sDisplay = NULL;
  sScreen = NULL;
  return w;
}

